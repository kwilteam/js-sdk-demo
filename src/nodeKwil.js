...
const kwil = new kwiljs.default.NodeKwil({
...
let db = new kwiljs.default.DBBuilder("demo_db", wallet.address);

// Create New Table
let usersTable = db.newTable("users", wallet)

    // Create columns
    let idColumn = usersTable.newColumn("id", kwiljs.default.Types.DataType.INT64);
    let userNameColumn = usersTable.newColumn("name", kwiljs.default.Types.DataType.STRING);
    let walletColumn = usersTable.newColumn("wallet", kwiljs.default.Types.DataType.STRING);

    // Add attributes to columns
    idColumn.addAttribute(kwiljs.default.Types.AttributeType.PRIMARY_KEY);
    userNameColumn.addAttribute(kwiljs.default.Types.AttributeType.NOT_NULL);
    userNameColumn.addAttribute(kwiljs.default.Types.AttributeType.MIN_LENGTH, 7);
    walletColumn.addAttribute(kwiljs.default.Types.AttributeType.NOT_NULL);
...
// Create Insert Query
let insertQuery = db.newQuery("insert_user", usersTable.name, kwiljs.default.Types.QueryType.INSERT);
    ...
    walletParameter.setModifier(kwiljs.default.Types.ModifierType.CALLER);
...
// Create Update Query
let updateQuery = db.newQuery("update_user", usersTable.name, kwiljs.default.Types.QueryType.UPDATE);
    ...
    let walletWhereClause = updateQuery.newWhere("wallet_where_clause", walletColumn.name, kwiljs.default.Types.OperatorType.EQUAL);

    // Add Caller Modifier to wallet where clause
    walletWhereClause.setStatic("");
    walletWhereClause.setModifier(kwiljs.default.Types.ModifierType.CALLER);
...
// Create Delete Query
let deleteQuery = db.newQuery("delete_user", usersTable.name, kwiljs.default.Types.QueryType.DELETE);

    // Add Where Clause to delete query
    let deleteWhereWallet = deleteQuery.newWhere("delete_wallet_where", walletColumn.name, kwiljs.default.Types.OperatorType.EQUAL);

    // Add Caller Modifier to delete Where Clause
    deleteWhereWallet.setStatic("");
    deleteWhereWallet.setModifier(kwiljs.default.Types.ModifierType.CALLER);
...
// Create New Index
let idIndex = db.newIndex("id_index", usersTable.name, kwiljs.default.Types.IndexType.BTREE);
...
async function deployDB(database) {
    ...
    let res = await kwil.broadcast(tx);
    console.log(res)
}
